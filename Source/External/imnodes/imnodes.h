...
struct ImNodesIO
{
    struct EmulateThreeButtonMouse
    {
        ...
        // The keyboard modifier to use in combination with mouse left click to pan the myEditor view.
    ...
    // Panning speed when dragging an element and mouse is outside the main myEditor view.
...
// An myEditor context corresponds to a set of nodes in a single workspace (created with a single
// Begin/EndNodeEditor pair)
//
// By default, the library creates an myEditor context behind the scenes, so using any of the imnodes
...
// will result the node myEditor grid workspace being rendered.
void BeginNodeEditor();
void EndNodeEditor();

// Add a navigable minimap to the myEditor; call before EndNodeEditor after all
...
// * myEditor space coordinates -- the origin is the upper left corner of the node myEditor window
// * grid space coordinates, -- the origin is the upper left corner of the node myEditor window,
// translated by the current myEditor panning vector (see EditorContextGetPanning() and
...
// Returns true if the current node myEditor canvas is being hovered over by the mouse, and is not
...
// myEditor. Use after calling EndNodeEditor().
...
// selection. Note that all functions require the id to be an existing valid id for this myEditor.
...
// Use the following functions to write the myEditor context's state to a string, or directly to a
// file. The myEditor context is serialized in the INI file format.
